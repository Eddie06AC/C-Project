...
Ñash_register::Ñash_register(ObjectPool* pool) :pool(pool)
{
	state = new Handle_clients(this);
}
void Ñash_register::change_state(State* state)
{
	if (this->state)
		delete this->state;
	this->state = state;
...
void Ñash_register::Do_handle(Client** mas, int length, int flagg[], float rate)
{
	state->do_Business(mas, length, flagg, rate);
}

Ñash_register_Simple::Ñash_register_Simple(ObjectPool* pool) : Ñash_register(pool), name("name"), max_people(1), available(1), served(0), card(0), cash(0), profit(0) {}

Ñash_register_Simple::Ñash_register_Simple(string name, int max_people, ObjectPool* pool) : Ñash_register(pool), available(1), served(0), card(0), cash(0), profit(0) { this->name = name; this->max_people = max_people; }
...
Ñash_register_Decorator::Ñash_register_Decorator(Ñash_register* decorated_cash_register, ObjectPool* pool) :Ñash_register(pool)
{
...
void ObjectPool::handle(Ñash_register* other, Client** mas, int length, int flagg[], float rate)
{
